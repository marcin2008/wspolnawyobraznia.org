    ...
    <main className="overflow-hidden bg-background text-foreground">
      ...
      <section id="fundacja" className="section-pad bg-paper">
        ...
        <div className="mx-auto grid max-w-7xl gap-14 px-5 lg:grid-cols-[0.85fr_1.15fr] lg:px-10">
          ...
          <div data-reveal className="reveal lg:pt-14"><p className="lead">Wspieramy czytelnictwo i rozwój kulturalny w Polsce. Chcemy, by dobra książka była naturalną częścią codzienności każdego dziecka.</p><div className="mt-10 grid gap-5 sm:grid-cols-3">{[[BookOpen,"Czytelnictwo","Budujemy trwałą relację dzieci z książką."],[Sprout,"Rozwój","Rozbudzamy ciekawość i samodzielność."],[MapPin,"Bliskość","Działamy tam, gdzie dzieci spędzają czas."]].map(([Icon,t,d])=><div className="border-t border-foreground/25 pt-5" key={String(t)}><Icon className="mb-5 text-primary"/><h3 className="font-display text-xl">{t as string}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{d as string}</p></div>)}</div></div>
        </div>
      </section>
